...
const Invoices = () => {
  const { isLoading, documents } = useDocuments(DocumentType.INVOICE);
  ...
  return (
    <>
      <ScreenHeader title={title} setOpen={() => setOpenCreate(true)}>
        <Link className={styles.btn_new} to={PrivateRoutes.NEW_INVOICE}>
          Nueva Factura
        </Link>
      </ScreenHeader>
      <ScreenContainer
        ...
        {documents?.items?.length <= 0 ? (
          <Empty
            button={''}
            title={title}
            copy="Aquí es donde puede administrar  sus facturas  y toda su información."
            image={Image}
          />
        ) : (
          <ReferralsTable />
